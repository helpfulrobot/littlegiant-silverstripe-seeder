...
class DataTypeProvider extends Provider
{
    ...
    protected function generateField($field, $state)
    {
        ...
        } elseif (strpos($dataType, 'currency') === 0) {
            $min = 0;
            $max = 1000;
            if (!empty($args['range'])) {
        ...
        } elseif (strpos($dataType, 'date') === 0) {
            return date('Y-m-d');
        } elseif (strpos($dataType, 'time') === 0) {
            return date('H:i:s');
        } elseif (strpos($dataType, 'ss_datetime') === 0) {
